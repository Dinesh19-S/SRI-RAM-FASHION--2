...
export default function CustomersPage() {
  ...
  function empty() { return { name:'', phone:'', email:'', address:'', gstin:'' }; }

  const load = useCallback(async () => {
    setLoading(true);
    try {
      const r = await api.get('/api/customers');
      setItems(r.data.data || r.data.customers || []);
  ...
  const openEdit = (c) => { setEditing(c); setForm({name:c.name||'',phone:c.phone||'',email:c.email||'',address:c.address||'',gstin:c.gstin||''}); setError(''); setShowModal(true); };

  const handleSave = async (e) => {
    e.preventDefault(); setSaving(true); setError('');
    try {
      if (editing) await api.put(`/api/customers/${editing._id}`, form);
  ...
  return (
    <div>
      <div className="page-header">
        <div><h1 className="page-title">Customers</h1><p className="page-subtitle">{items.length} customers</p></div>
      ...
      {loading ? <div className="loading-center"><div className="spinner"/></div> : (
        <div className="card" style={{padding:0}}>
          <div className="table-wrapper">
            <table>
              <thead><tr><th>Name</th><th>Phone</th><th>Email</th><th>GSTIN</th><th>Actions</th></tr></thead>
              <tbody>
                {filtered.length === 0 ? (
                  <tr><td colSpan={5}><div className="empty-state"><div className="empty-icon">👤</div><p>No customers</p></div></td></tr>
                ) : filtered.map(c => (
                  <tr key={c._id}>
            ...
            <form onSubmit={handleSave}>
              <div className="modal-body">
                ...
                <div className="form-row">
                  <div className="form-group"><label className="form-label">Name *</label><input className="form-input" value={form.name} onChange={e=>s('name',e.target.value)} required /></div>
